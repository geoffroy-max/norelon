...
{
    /**
     * cette fonction permet de creer une session stripe en fonction de la reference
    ...
    public function index(Cart $cart,$reference,EntityManagerInterface $em)
    {
        ...
        if (!$order3){
            $this->redirect('order3');
        }

        //bouclé sur nos données en provenance de la bd de orderdetail3

        foreach($order3->getOrderDetail3s()->getValues() as $product) {
        ...
        $product_for_stripe[] = [
            'price_data' => [
                'currency' => 'eur',
                'unit_amount' => $order3->getCarrierPrice(),
                'product_data' => [
                    'name' => $order3->getCarrierName(),
                    'images' => [$YOUR_DOMAIN],
                ],
            ],
            'quantity' => 1,
...
//?session_id=
        ...
        $order3->setStripeSessionId($checkout_session->id);
        $em->flush();
